...
# - .env 里 CLOUDFLARE_TUNNEL_TOKEN 有值：跑命名隧道（固定域名，在 Cloudflare Zero Trust 里建好后拿 token）
# - 没有 token：跑临时隧道，得到一个随机 https://xxx.trycloudflare.com 网址（每次重启会变），写到 data/tunnel_url.txt，
#   侧栏「后台运行」面板会显示当前网址
