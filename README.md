...
## 4.2c 推荐的三道门槛（性能不降 / 真降本 / 买得到）

`recommend_schemes` 出方案时按顺序过滤，参数在 `RecommendInput`：
1. **买得到**（`only_buyable`）：只用有报价来源的料，现配方在用的料与必配助剂例外（标待询价）；文献里的实验室材料不会进方案。
2. **性能相当**（`require_parity`）：定性评估后，任一项 ↓↓ 或累计两项 ↓ 的方案不推荐（`qualitative.parity`）；没做评估的标“需小试验证”，不算降性能。
3. **真降本**（`min_savings_pct`，默认 3%）：降本不到 3% 不值得改配方。
4. **档位分散**（`diversify`）：按再生料比例分成 稳妥 <30% / 中等 30～45% / 激进 ≥45%，同一档最多占一半，
   notes 里给出档位分布；某档没有达标方案时说明原因（例如同类 LLDPE 已无价差时，降本只能靠再生料）。
